...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'background': '#ffffff', 'primaryColor': '#B8E0F6', 'primaryTextColor': '#000000', 'primaryBorderColor': '#4A90E2', 'lineColor': '#333333', 'tertiaryColor': '#ffffff'}}}%%
graph TD
    %% 第一行：用户实体
    subgraph UsersSection[" "]
        direction TB
        UserId((id))
        UserName((username))
    ...
    %% 第二行：用户关系
    Users -->|1| PublishRel{发表评论}
    Users -->|N| FavoriteRel{收藏电影}
    
    %% 第三行：评论和收藏实体
    subgraph CommentsSection[" "]
        direction TB
        CommentId((id))
        CommentUserId((user_id))
        Comments[评论表<br/>COMMENTS]
        CommentMovieId((movie_id))
        CommentParentId((parent_id))
        CommentContent((content))
        
        CommentId --> Comments
        CommentUserId --> Comments
        Comments --> CommentMovieId
        Comments --> CommentParentId
        Comments --> CommentContent
    end
    
    subgraph FavoritesSection[" "]
        direction TB
        FavId((id))
        FavUserId((user_id))
        Favorites[收藏表<br/>FAVORITES]
        FavMovieId((movie_id))
        FavCreated((created_at))
        
        FavId --> Favorites
        FavUserId --> Favorites
        Favorites --> FavMovieId
        Favorites --> FavCreated
    end
    
    PublishRel -->|N| Comments
    FavoriteRel -->|M| Favorites
    
    %% 第四行：电影关系
    Comments -->|N| HasCommentRel{包含评论}
    Favorites -->|N| IsFavoritedRel{被收藏}
    
    HasCommentRel -->|1| Movies
    IsFavoritedRel -->|1| Movies
    
    %% 第五行：电影实体
    subgraph MoviesSection[" "]
        direction TB
        MovieId((id))
        MovieTitle((title))
        MovieDesc((description))
        Movies[电影表<br/>MOVIES]
        MovieGenre((genre))
        MovieYear((year))
        MovieRating((rating))
    ...
    style HasCommentRel fill:#FFE6CC,stroke:#FF9933,stroke-width:2px,color:#000000
    style FavoriteRel fill:#FFE6CC,stroke:#FF9933,stroke-width:2px,color:#000000
    style IsFavoritedRel fill:#FFE6CC,stroke:#FF9933,stroke-width:2px,color:#000000
    
    style UsersSection fill:none,stroke:none
    style CommentsSection fill:none,stroke:none
    style FavoritesSection fill:none,stroke:none
    style MoviesSection fill:none,stroke:none
```
